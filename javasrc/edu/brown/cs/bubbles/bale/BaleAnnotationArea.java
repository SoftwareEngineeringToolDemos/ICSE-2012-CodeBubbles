...
@Override public void paintComponent(Graphics g)
{
   ...
   if (!BALE_PROPERTIES.getBoolean(BALE_EDITOR_DO_GRADIENT)) {
      ...
      p = new GradientPaint(0f,0f, BALE_PROPERTIES.getColor(BALE_EDITOR_TOP_COLOR), 0f, bnd.height, BALE_PROPERTIES.getColor(BALE_EDITOR_BOTTOM_COLOR));
    }
   ...
   try {
	 ...
	 if (lno - start_line < 0 || lno - start_line >= line_data.length) {
	    BoardLog.logE("BALE","Annotation beyond bounds " + lno + " " + start_line + " " + line_data.length);
	    continue;
	  }
	 ...
	 for (BaleAnnotation an : lan) {
	    ...
	    int x = bnd.x + bnd.width/2 - wid/2;//can't test because breakpoint doesn't work
	    int y = (y0 + y1)/2 - ht/2;
