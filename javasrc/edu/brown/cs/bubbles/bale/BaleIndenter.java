...
class BaleIndenter implements BaleConstants {


// FIXES NEEDED:
//   * components of IF need to be indented inside the IF
...
private BoardProperties 	bale_props;
private BumpClient		bump_client;
private BaleDocument		bale_document;
private Segment 		doc_text;
...
BaleIndenter(BaleDocument bd)
{
   bump_client = BumpClient.getBump();
   bale_props = BoardProperties.getProperties("Bale");
   bale_document = bd;
...
boolean skipScope(BaleTokenType open,BaleTokenType close)
{
   int ct = 0;
   for ( ; ; ) {
...
@SuppressWarnings("fallthrough")
int skipToStatementStart(boolean danglingelse,boolean isinblock)
{
   final int NOTHING = 0;
...
private int getLineIndent(int lno)
{
...
private int getLeadingWhitespaceLength(int offset)
{
...
private int getPositionWhitespaceLength(int offset)
{
   StringBuffer indent = new StringBuffer();
   int lno = bale_document.findLineNumber(offset);
   int lstart = offset;
...
{
   final int tabsize = pref_tab_size;
   int length = 0;
...
{
   int soff = cur_element.getStartOffset();
   int eoff = cur_element.getEndOffset();
...
BaleElement getNextElement(BaleElement e,boolean stopateol)
{
...
BaleElement getPreviousElement(BaleElement e)
{
   if (e == null) return null;
...
BaleElement findOpeningPeer(BaleElement e,BaleTokenType open,BaleTokenType close)
{
   int ct = 0;
   for ( ; ; ) {
...
BaleTokenType getToken(BaleElement e)
{
...
{
   String v = bale_props.getProperty("Bale.indent." + id);
   if (v != null) return v;
